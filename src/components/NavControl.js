...
const NavControl = ({ isOpen = false, currentNavContentId, onShowMenu }) => {
  ...
  return (
    <nav className={renderNavControlClass}>
      <div className={renderBtnListClass} onClick={() => handleNavBtn(cons.LIST_SECTION)}></div>
      <div className={renderBtnChartClass} onClick={() => handleNavBtn(cons.CHART_SECTION)}></div>
      <div className={renderBtnConfigClass} onClick={() => handleNavBtn(cons.CONFIG_SECTION)}></div>
    </nav>
  )
}
